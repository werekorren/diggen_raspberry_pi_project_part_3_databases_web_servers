...
We use an \emph{IP address} to specify the computer, or \emph{host}, on which the service is running. In this exercise we will be running the Redis service on the same computer as the programs using the database and we can therefore for now use ''localhost'' or ''127.0.0.1'' as the IP address which means that the service runs on our local computer. You will encounter other IP addresses later in the course when we will connect to other computers in a network. If you are interested in learning more, see \cite{ipaddress}.
...
By default, the Redis server will start automatically after it is installed by the command above. 
 To test if the redis server is running, type the following in a terminal:
\begin{equation}
...
\begin{equation}
    \texttt{redis-server}
\end{equation}
This will start the redis server on its default port 6379. If you want it to run on a different port, for example on port 7777, use
\begin{equation}
...
\noindent You can repeat this command with different value input for the same key name to change its value.\\
...
In order to be able to use our database in Python, we need to install the \textbf{Redis Python module}, so that we can use it to communicate with our Redis database. Use the following command to install the Redis Python module:
\begin{equation}
...
\begin{itemize}
    ...
    \item[1.] Try Redis out, using the Python code below (we suggust you run it in Thonny):
...
Browsers, such as \emph{Chrome} or \emph{Safari}, are programs that retrieve and display web pages. Web pages are \textit{hosted} on different servers, so every time you want to go to a new web page, your browser needs to communicate with the server where the new page is hosted \cite{web}. 

When you type a web address/URL, let us say \url{www.xkcd.com}, your browsers submit an HTTP request to xkcd's server, and the server starts to send over the information needed to display the web page. Your browser processes all the information which contains several different types of code (typically at least HTML, XML or JavaScript). When your browser has processed all the components, it displays the web page \cite{web}.
\begin{figure}[h]
    \centering
...
We must start by installing Flask. Open a terminal and type

\begin{equation}
    \texttt{sudo apt install python3-flask}
\end{equation}


\noindent Then follow {\color{blue} \href{https://projects.raspberrypi.org/en/projects/python-web-server-with-flask/0}{this tutorial}} (https://projects.raspberrypi.org/en/projects/python-web-server-with-flask/0).

In the tutorial you will learn how to make a web application and how to add web pages to it. Add more pages and find more information about HTML and CSS to make your pages look better. Add at least one image to one of your pages -- use the \verb!<img>! tag to include an image object on your web page.
When you are done, your web application should consist of at least two web pages that you can navigate between (by, say, clicking on links embedded in the web pages). Your CCS styling does not need to be pretty. We are just interested in you getting used to using it in practice, so any (visible) styling will do for now.\\
...
{\bf Hint 1: }\parbox[t]{14cm}{To get rid of the ugly \texttt{b''} surrounding your values you retrieve from your Redis-database, you need to use the \texttt{decode()} method or configure your Redis connection to automatically decode any response.}\\
...
\bibitem{loved} Developer Survey Results 2021: Most Loved, Dreaded, and Wanted Databases, \url{https://insights.stackoverflow.com/survey/2021#section-most-loved-dreaded-and-wanted-databases}, last accessed on 2025-10-30.
...
\bibitem{ipaddress} IP Address, \url{https://en.wikipedia.org/wiki/IP_address}, last accessed on 2025-10-30.
\bibitem{portnumber} Port (computer networking), \url{https://en.wikipedia.org/wiki/Port_(computer_networking)}, last accessed on 2025-10-30.
\bibitem{web} Web browser, \url{https://en.wikipedia.org/wiki/Web_browser}, last accessed on 2025-10-30.
\bibitem{cookie} HTTP cookie, \url{https://en.wikipedia.org/wiki/HTTP_cookie}, last accessed on 2025-10-30.
\bibitem{serv} Web server -- Wikipedia, The Free Encyclopedia, \url{https://en.wikipedia.org/wiki/Web_server}, last accessed on 2025-10-30.
\bibitem{bottle} Flask (web framework) -- Wikipedia, The Free Encyclopedia, \url{https://en.wikipedia.org/wiki/Flask_(web_framework)}, last accessed on 2025-10-30.
