...
When you type a web address/URL, let us say \url{www.xkcd.com}, your browsers submit an HTTP request to xkcd's server, and the server starts to send over the information needed to display the web page. Your browser processes all the information which contains several different types of code (typically at least HTML, XML or JavaScript) and elements (such as Flash or ActiveX). When your browser has processed all the components, it displays the web page \cite{web}.
\begin{figure}[h]
    \centering
...
When you are done, your web application should consist of at least two web pages that you can navigate between (by, say, clicking on links embedded in the web pages). AYour CCS styling does not need to be pretty. We are just interested in you getting used to using it in practice, so any (visible) styling will do for now.\\
